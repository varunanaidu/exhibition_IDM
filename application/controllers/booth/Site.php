...
class Site extends CI_Controller {
	...
	public function save_attendance()
	{
		...
		$check = $this->sitemodel->view('view_booth_attendance', '*', ['registration_id'=>$registration_id, 'booth_id'=>$booth_id, 'DATE(attendance_date)'=>date('Y-m-d')]);
		if ($check) {
			$this->response['msg'] = 'Anda telah mengunjungi booth ini pada tanggal <br> ' . date('d F Y H:i:s', strtotime($check[0]->attendance_date));
